import { RAMADAN_YEAR_AH, getRamadanDay } from '../lib/ramadan';
...
export function Header({ locationError, todayPrayerTimes, sunProgress }: HeaderProps) {
    ...
    <div className="text-center mb-8 pb-6 border-b border-white/[0.06]">
      ...
      {todayPrayerTimes && (
        <>
          ...
          {/* Sun arc during daytime, moon phase at night */}
          <div className="mt-4">
            {isDaytime ? (
              <SunArc
                sunProgress={sunProgress}
                fajrTime={todayPrayerTimes.fajr}
                maghribTime={todayPrayerTimes.maghrib}
              />
            ) : (
              <MoonPhase ramadanDay={ramadanDay} />
            )}
          </div>
        </>
      )}
